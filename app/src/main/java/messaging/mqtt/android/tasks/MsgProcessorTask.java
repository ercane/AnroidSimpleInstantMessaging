...
public class MsgProcessorTask implements Runnable {
    ...
    public void run() {
        try {
            Log.d(TAG, "Message arrived"+ new String(payload,"utf-8"));
            ...
            } else if (payloadMsg.startsWith(MqttConstants.MQTT_MSG_SENT)) {
                String[] split = payloadMsg.split(MqttConstants.MQTT_SPLIT_PREFIX);
                if (!Build.ID.equals(split[1])) {
                    byte[] decryptMsg = MsgEncryptOperations.decryptMsg(topic, split[2].getBytes());
